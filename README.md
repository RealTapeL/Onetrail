...
本机访问 <http://localhost:5173>；局域网内其他设备访问 `http://<本机IP>:5173`（当前为 <http://192.168.0.109:5173>，IP 变化时以 `ip addr` 为准）。后端 API 也可直接通过 `http://<本机IP>:8000/docs` 访问。首次打开会进入登录页，可直接注册新账号；种子脚本使用的演示账号为 `admin@onetrail.dev / admin123456`。
