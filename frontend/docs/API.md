...
- **新增** `GET /api/v1/meta/geocode?city=`：城市/地名 → `{latitude, longitude, formatted_address}`（高德地理编码）。S1/M1 的目的地输入在提交时经此解析坐标，用户不再接触经纬度。
- **推荐响应新增 `hiking_spots`**：当路线库在目的地 100km 内没有路线时，返回高德 POI 文本搜索得到的真实周边徒步地 `{name, category, address, distance_m, location}`（风景名胜/公园广场分类 + 徒步关键词过滤，最多 8 条；distance_m 为与目的地的直线距离）。仅位置信息，**无路线参数**，前端以独立区块展示并注明来源。
